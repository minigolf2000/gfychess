...
export class ChessGif {
  ...
  applyMove(moves: string[], idx: number, board: string[][]): number[][] {
    let side = 'w';
    let offset = 1;
    if ((idx & 1) === 1) {
      side = 'b';
    ...
    if (move.length === 2) {
      ...
      board[rank][files[0]] = ' ';
      board[rank][files[1]] = ' ';
      board[rank][files[2]] = king;
      board[rank][files[3]] = rook;
      return files.map((file) => [rank, file]);
      ...
      for (const d of directions) {
        for (let i=1; i<8; i++) {
          ...
          if (board[r2][f2] === piece) {
            candidates.push([r2, f2]);
            break;
          }
          if (board[r2][f2] != ' ') break;
        }
      }
    ...
    for (const c of candidates) {
      ...
      if (y < 8 && y >= 0 && x < 8 && x >= 0 && board[y][x] === piece) {
  ...
  drawSquare(boardRaster: Uint8Array, y: number, x: number, piece: string) {
    const bg = (y + x) % 2;
    ...
    if (piece === ' ') {
      for (let i=0; i<SQUARE_SIZE; i++) {
        ...
        for (let j=0; j<SQUARE_SIZE; j++) {
          boardRaster[boardY + j + x*SQUARE_SIZE] = bg;
        }
      }
    } else {
      const color = piece[0];
      ...
      for (let i=0; i<SQUARE_SIZE; i++) {
        ...
        const boardY = (y*SQUARE_SIZE + i) * BOARD_SIZE;
        for (let j=0; j<SQUARE_SIZE; j++) {
          boardRaster[boardY + j + x*SQUARE_SIZE] = PIECE_DATA[srcY + xOff + j];
        }
      }
    }
  }

  drawRegion(boardRaster: Uint8Array, sequence: Uint8Array, y: number, xMin: number, xMax: number): Uint8Array {
    y *= SQUARE_SIZE;
    xMin *= SQUARE_SIZE;
    xMax *= SQUARE_SIZE;
    ...
    for (let i=y; i<y+SQUARE_SIZE; i++) {
      for (let j=xMin; j<xMax; j++) {
        sequence[idx] = boardRaster[i*BOARD_SIZE + j];
        idx++;
      }
    }

    return sequence.subarray(0, space);
  }

  drawBoard(board: string[][]): Uint8Array {
    const boardRaster = new Uint8Array(BOARD_SIZE * BOARD_SIZE);
    for (let i=0; i<8; i++) {
      boardRaster.set(ROWS[i&1], i * BOARD_SIZE * SQUARE_SIZE);
      for (let j=0; j<8; j++) {
        this.drawSquare(boardRaster, i, j, board[i][j]);
      }
    }
  ...
  createGif(moves: string[]): Uint8Array {
    const board = [
    ...
    const boardRaster = this.drawBoard(board);
    const boardCache: {[key: string]: Uint8Array} = {};
    const sequence = new Uint8Array(BOARD_SIZE * SQUARE_SIZE);
    ...
    while (true) {
      if (xMin !== 8) {
        ...
        const delay = moveIdx === moves.length ? 200 : 100;
        this.startImage(xMin * SQUARE_SIZE, yMin * SQUARE_SIZE, width, height, delay);

        for (let i=yMin; i<yMax; i++) {
          const skew = (i + xMin) & 1;
          const key = board[i].slice(xMin, xMax).join('') + skew.toString();
          if (boardCache[key] === undefined) {
            const region = this.drawRegion(boardRaster, sequence, i, xMin, xMax);
            boardCache[key] = this.lzw(region);
          } else {
            this.gif.data.set(boardCache[key], this.gif.idx);
            this.gif.idx += boardCache[key].length;
          }
        }
      ...
      for (const affected of this.applyMove(moves, moveIdx, board)) {
        const y = affected[0];
        const x = affected[1];
        this.drawSquare(boardRaster, y, x, board[y][x]);
...
async function main() {
  ...
  try {
    const gif = chessGif.createGif(moves);
    const end = Date.now();

    console.log("data:image/gif;base64, " + Buffer.from(gif).toString('base64'));

    fs.writeFile("test.gif", gif, (err) => console.log(`Time elapsed: ${end - begin}ms`));
  } catch(e) {
    console.log(e);
  }
}
