...
export function MoveSelector({
  ...
  const className = (i: number) => {
    const className = [];
    const [min, max] = currentHoveringRange();
  ...
  const onMouseMove = (e: MouseEvent) => {
    if (isDraggingStartHandle || isDraggingEndHandle) {
      ...
      nextHoveredIndex = Math.min(
        Math.max(0, nextHoveredIndex),
        moves.length - 1
      );
  ...
  const tableClassName =
    mouseDownIndex > -1 && !isDraggingStartHandle && !isDraggingEndHandle
      ? "table mousedown"
      : "table";
  const [startHandleIndex, endHandleIndex] = currentHoveringRange();
  const startHandleStyle: React.CSSProperties = {
    top: Math.floor(startHandleIndex / 2) * ROW_HEIGHT + 1,
    left: (startHandleIndex % 2) * ROW_WIDTH - DRAGGABLE_HANDLE_WIDTH + 35,
  };
  const endHandleStyle: React.CSSProperties = {
    top: Math.floor(endHandleIndex / 2) * ROW_HEIGHT + 1,
